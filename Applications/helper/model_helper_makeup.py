...
def get_model_function(output_directory, network_name, num_classes, initial_checkpoint=None, checkpoint_exclude_scopes=None, ignore_missing_variables=False, trainable_scopes=None,
	...
	def model_fn(features, labels, mode, params):
		...
		with tf.variable_scope('generator'):
			...
			input_6ch = tf.concat([images, targets_out], 3)
			output_1 = build_generator_resnet_9blocks_tf(input_6ch,  targets_out, out_channels, batch=images.get_shape().as_list()[0], name='generator_main', ngf=64, rate=1)#RandomNet(targets_out, name='RandomNet')
		...
		if mode == ModeKeys.EVAL or mode == ModeKeys.TRAIN:
			...
			with tf.name_scope('losses'):
				...
				with tf.name_scope("New_Gen_Loss"):
					...
					loss2 = spl_loss.GP(output_1, images)
					loss1 = spl_loss.CP(output_1, targets_out)
...
def build_generator_resnet_9blocks_tf(inputgen,ref, out_channels, batch=12, name="generator", skip=True, ngf=32, reuse=False, rate=1):
...
#        print(o_r9.get_shape().as_list())
		...
		o_c6 = layers.general_conv2d(o_c5, out_channels, f, f, 1, 1,
									 0.02, "SAME", "c6",
									 do_norm=False, do_relu=False, rate=rate) ## add VALID instead of 'same' for dil in conv1-rate=4 252x252
 #       o_c6 = tf.pad(o_c6, [[0, 0], [1, 3], [2, 2], [0, 0]], "SYMMETRIC") ## added padding for dialted 4 [3 2] to make it 256x256
		if skip is True:
